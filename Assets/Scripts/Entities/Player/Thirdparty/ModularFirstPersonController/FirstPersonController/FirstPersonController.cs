...
public class FirstPersonController : MonoBehaviour, IService
{
    ...
    private Vector2 Movement => InputController.Instance.GetValue<Vector2>(InputPatterns.Movement);
    private Vector2 MouseDelta => InputController.Instance.GetValue<Vector2>(InputPatterns.MouseDelta);

    private bool JumpTriggerd => InputController.Instance.Triggered(InputPatterns.Jump);
    ...
    private void Update()
    {
        ...
        if(enableJump && JumpTriggerd && isGrounded)
        {
            Jump();
        }
    ...
    public void SetJumpMulitiplier(float val)
    {
        jumpMultiplier = val;
    }
    ...
    private void CheckGround()
    {
        ...
        if (Physics.Raycast(origin, direction, out RaycastHit hit, distance, groundedMask))
        {
    ...
    private void Jump()
    {
        ...
        if (isGrounded) {
            rb.velocity += jumpPower / jumpMultiplier * Vector3.up;
            isGrounded = false;
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.groundedMask = EditorGUILayout.LayerField(new GUIContent("Ground Mask", "Checks the ground for these layers"), fpc.groundedMask);
        GUI.enabled = true;
